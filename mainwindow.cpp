...
void MainWindow::drawSmoothLine(QPainter &painter, int x1, int y1, int x2, int y2)
{//алгоритм Брезенхема сглаживания
    int dx = abs(x2 - x1);
    int dy = abs(y2 - y1);
    int sx = (x1 < x2) ? 1 : -1;
    int sy = (y1 < y2) ? 1 : -1;
    ...
    // Определение начальной и конечной интенсивности цвета
    int startIntensity = 255;
    int endIntensity = 0;

    while (true)
    {
        // Рисование текущего пикселя с учетом интенсивности
        QColor color(0, 0, 0, startIntensity);
        painter.setPen(color);
        painter.drawPoint(x1, y1);

        // Завершение цикла, когда достигнут конечный пиксель
        if (x1 == x2 && y1 == y2)
            break;
        ...
        // Обновление интенсивности цвета
        int distance = abs(x2 - x1) + abs(y2 - y1);
        startIntensity = 255 - (distance * (255 - endIntensity) / (dx + dy));
    }
}
...
void MainWindow::wheelEvent(QWheelEvent *event)
{
    ...
    // Определяем направление прокрутки (вперед или назад)
    if (!numDegrees.isNull())
    {
        QPoint numSteps = numDegrees / 15;

        // Определяем фактор масштабирования в зависимости от направления прокрутки
        double zoomFactor = 1.0;
        if (numSteps.y() > 0)
            zoomFactor = 1.2; // Увеличение масштаба
        else if (numSteps.y() < 0)
            zoomFactor = 0.8; // Уменьшение масштаба

        // Изменяем размеры QPixmap с учетом зума
        int newPixWidth = pix.width() * zoomFactor;
        int newPixHeight = pix.height() * zoomFactor;
        pix = pix.scaled(newPixWidth, newPixHeight);

        // Обновляем графический элемент label
...
{
    int initialPixWidth = pix.width();
    int initialPixHeight = pix.height();

    double x0 = ui->doubleSpinBox->value() * (pix.width() / initialPixWidth);
    double x1 = ui->doubleSpinBox_2->value() * (pix.width() / initialPixWidth);
    double y0 = ui->doubleSpinBox_3->value() * (pix.height() / initialPixHeight);
    double y1 = ui->doubleSpinBox_4->value() * (pix.height() / initialPixHeight);
    ...
    drawSmoothLine(p, pix.width()*0.05,pix.height()/2,pix.width()*0.95,pix.height()/2);
    ...
    for(int i = 0; i < 20;i++)
    {
        ...
        p.drawText(pix.width()*(0.04+step*20), pix.height()/1.90, "X"); // Подпись оси X
        p.drawText(pix.width()/2.10, pix.height()*(0.955-step*20.5), "Y"); // Подпись оси Y
        if(i!=10)
    ...
    if(algorithm != BRESENHAM_CIRCLE) {
        ...
        {
            //drawSmoothLine(p, pix.width()*(0.5+stp*line[i-1].x()),pix.height()*(0.5-stp*line[i-1].y()),pix.width()*(0.5+stp*line[i].x()),pix.height()*(0.5-stp*line[i].y()));
        ...
        for(int i = 0; i<4;i++)
        {
            ...
            {
                drawSmoothLine(p, pix.width()*(0.5+stp*circle[i][j-1].x()),pix.height()*(0.5-stp*circle[i][j-1].y()),pix.width()*(0.5+stp*circle[i][j].x()),pix.height()*(0.5-stp*circle[i][j].y()));
